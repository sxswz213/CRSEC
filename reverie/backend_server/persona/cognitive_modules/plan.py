...
from norm.norm_retrieve import *
from norm.norm_compliance import *
      ...
      for count, curr_hour_str in enumerate(hour_str): 
        if wake_up_hour > 0: 
          ...
          n_m1_activity += [run_gpt_prompt_generate_hourly_schedule(
                          persona, curr_hour_str, n_m1_activity, hour_str, curr_act_norms)[0]]
...
def generate_task_decomp(persona, task, duration): 
  ...
  for norm_id, a_norm in persona.norm_database.act_norm.items():
    ...
    curr_act_norms += a_norm.content
    curr_act_norms += "\n"
  return run_gpt_prompt_task_decomp(persona, task, duration, curr_act_norms)[0]

def generate_task_decomp_v2(persona, task, duration): 
  """
  A few shot decomposition of a task given the task description 
  ...
  for norm_id, a_norm in persona.norm_database.act_norm.items():
    ...
    curr_act_norms += a_norm.content
    curr_act_norms += "\n"
  return run_gpt_prompt_task_decomp_v2(persona, task, duration, curr_act_norms)[0]
...
def generate_convo(maze, init_persona, target_persona, norm_conf_res): 
  curr_loc = maze.access_tile(init_persona.scratch.curr_tile)

  # convo = run_gpt_prompt_create_conversation(init_persona, target_persona, curr_loc)[0]
  # convo = agent_chat_v1(maze, init_persona, target_persona)
  convo = agent_chat_v2(maze, init_persona, target_persona, norm_conf_res)
  all_utt = ""
...
def _long_term_planning(persona, new_day): 
  ...
  if new_day == "First day": 
    ...
    revise_identity_v2(persona)

    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - TODO
    # We need to create a new daily_req here...
    persona.scratch.daily_req = generate_new_daily_plan(persona, 
                                                          wake_up_hour)
  ...
  persona.scratch.norm_importance_trigger_curr -= thought_poignancy
...
def _determine_action(persona, maze): 
  ...
  print("currrrrrrrrr_index: ", curr_index)
  if curr_index == 0 or persona.scratch.curr_time.strftime("%H:%M:%S") == "09:00:00":
    ...
    if act_dura >= 60: 
      ...
      if determine_decomp(act_desp, act_dura): 
        persona.scratch.f_daily_schedule[curr_index:curr_index+1] = (
                            generate_task_decomp_v2(persona, act_desp, act_dura))
  ...
  print("currrrrrrrrr_index_60: ", curr_index_60)
  curr_index_60 = persona.scratch.get_f_daily_schedule_index(advance=60)
  print("currrrrrrrrr_index_60: ", curr_index_60)
...
def _choose_retrieved(persona, retrieved, retrieved_norm): 
  """
  ...
  # Always choose norm conflict first.
  if retrieved_norm:
    priority = []
    for event_desc, rel_ctx in retrieved.items(): 
      ...
      if (":" not in curr_event.subject 
        and curr_event.subject != persona.name): 
          x = check_if_norm_conflict(retrieved_norm, event_desc, curr_event.subject, persona)
          if x[0]:
            priority += [[rel_ctx, x[1]]]
    if priority: 
      e=random.choice(priority)
      persona.scratch.norm_conflict = True
      return e[0], True, e[1]
  ...
  for event_desc, rel_ctx in retrieved.items(): 
    ...
    if (":" not in curr_event.subject 
      ...
      priority += [rel_ctx]
  if priority: 
    return random.choice(priority), False, "None"
...
def _should_react(persona, retrieved, personas, norm_conflict): 
  """
  ...
  """
  def lets_talk(init_persona, target_persona, retrieved, norm_conflict):
  ...
  if ":" not in curr_event.subject: 
    ...
    if lets_talk(persona, personas[curr_event.subject], retrieved, norm_conflict):
...
def _chat_react(maze, persona, focused_event, reaction_mode, personas, norm_conf_res):
  ...
  convo, duration_min = generate_convo(maze, init_persona, target_persona,norm_conf_res)
...
def plan(persona, maze, personas, new_day, retrieved, perceived): 
  """
  ...
  # print("PART 3 Step 1")
  if retrieved.keys(): 
    # print("PART 3 Step 1: retrieved not null")
    retrieved_norm = norm_retrieve(persona, perceived)
    focused_event, norm_conflict, norm_conf_res = _choose_retrieved(persona, retrieved, retrieved_norm)
  ...
  #         c) False
  if focused_event: 
    reaction_mode = _should_react(persona, focused_event, personas, norm_conflict)
    if reaction_mode: 
      # If we do want to chat, then we generate conversation 
      if reaction_mode[:9] == "chat with":
        _chat_react(maze, persona, focused_event, reaction_mode, personas, norm_conf_res)
