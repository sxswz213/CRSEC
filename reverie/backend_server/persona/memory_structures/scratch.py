...
class Scratch: 
  def __init__(self, f_saved): 
    ...
    # Norm reflection variables
    self.norm_importance_trigger_max = 50
    self.norm_importance_trigger_curr = self.norm_importance_trigger_max
    ...
    if check_if_file_exists(f_saved): 
      ...
      self.identity = scratch_load["identity"]
      self.norm_importance_trigger_max = scratch_load["norm_importance_trigger_max"]
      self.norm_importance_trigger_curr = scratch_load["norm_importance_trigger_curr"]
      self.norm_conflict = scratch_load["norm_conflict"]
      self.norm_count = scratch_load["norm_count"]
      self.norm_evaluate = scratch_load["norm_evaluate"]
      self.act_norm_count = scratch_load["act_norm_count"]
  ...
  def save(self, out_json):
    ...
    scratch["identity"] = self.identity
    scratch["norm_importance_trigger_max"] = self.norm_importance_trigger_max
    scratch["norm_importance_trigger_curr"] = self.norm_importance_trigger_curr
    scratch["norm_conflict"] = self.norm_conflict
    scratch["norm_count"] = self.norm_count
    scratch["norm_evaluate"] = self.norm_evaluate
    scratch["act_norm_count"] = self.act_norm_count
  ...
  def act_check_finished(self): 
    ...
    #if end_time.strftime("%H:%M:%S") == self.curr_time.strftime("%H:%M:%S") or 
    if end_time.strftime("%Y-%m-%d %H:%M:%S") < self.curr_time.strftime("%Y-%m-%d %H:%M:%S"): 
      return True
    return False
